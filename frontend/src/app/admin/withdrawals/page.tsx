...
import {
  Banknote, Copy, CheckCircle, XCircle, AlertCircle, Loader2, RefreshCw, ArrowLeft, Search, Filter,
} from 'lucide-react';
...
const TAB_STATUSES = ['PENDING', 'APPROVED', 'COMPLETED', 'REJECTED'] as const;
...
export default function AdminWithdrawalsPage() {
  ...
  const handleComplete = async (id: string) => {
    ...
    try {
      ...
      setActionMsg('Withdrawal completed — gift code provided to user');
  ...
  const handleReject = async (id: string) => {
    if (!confirm('Reject this withdrawal request? User will be refunded.')) return;
    setApproving(id);
    setActionMsg('');
    try {
      await adminApi.reviewRedeem(id, 'REJECTED', { rejectionReason: 'Request denied by admin' });
      setActionMsg('Withdrawal rejected — funds returned to user wallet');
      await loadData();
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
          <div>
            ...
            <p className="text-zinc-400 mt-1">Approve or reject cash-out / gift-card redemptions</p>
          </div>
        ...
        {requests.length === 0 ? (
          <div className="glass-card rounded-2xl p-16 text-center">
            ...
            <p className="text-zinc-500">No {filter.toLowerCase()} withdrawal requests</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2 text-sm flex-wrap">
                      ...
                      <span className="text-zinc-400">{r.type === 'GIFT_CARD' ? 'Gift Card' : 'Bank Transfer'}</span>
                      {r.accountDetails && (
                        <>
                          <span className="text-zinc-600">|</span>
                          <span className="text-zinc-400 text-xs truncate max-w-[200px]">{r.accountDetails}</span>
                        </>
                      )}
                      <span className="text-zinc-600">|</span>
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                      <div className="flex gap-2">
                        <input
                          ...
                          placeholder="Gift code for user..."
