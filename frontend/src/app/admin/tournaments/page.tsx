...
export default function AdminTournamentsPage() {
  ...
  useEffect(() => {
    ...
    const b = calculateCommission(Number(form.entryFee), effectiveMaxParticipants, form.gameMode);
    setBreakdown(b);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                        ...
                        return (
                          <div className="space-y-3">
                            ...
                            <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-xs text-yellow-300">
                              🏆 <strong>Clash Squad Winner-Takes-All:</strong> 100% of the prize pool ({formatCurrency(maxPrizePool)}, 88% of total collection) is allocated directly to the winning team.
                            </div>
                          </div>
                        );
                      }
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div className="flex items-center gap-2 text-xs flex-wrap">
                      ...
                      {t.tournamentFormat === 'PER_KILL' && (
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-yellow-500/15 text-yellow-400 border border-yellow-500/25 ml-1">
      ...
      {previewScreenshot && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4" onClick={() => setPreviewScreenshot(null)}>
          <div
            className="bg-zinc-900 border border-white/10 rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 bg-white/[0.02]">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-fire-500/10 border border-fire-500/20 flex items-center justify-center text-fire-400">
                  <Shield className="w-4 h-4" />
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                    {previewScreenshot.username}
                    {previewScreenshot.isVerified && (
                  ...
                  <div className="flex items-center gap-2 text-xs text-zinc-400">
                    {previewScreenshot.ign && <span className="text-fire-400">IGN: {previewScreenshot.ign}</span>}
                    {previewScreenshot.freeFireId && (
                      <span className="font-mono bg-white/5 px-1.5 py-0.5 rounded text-[11px] text-zinc-300">
                        UID: {previewScreenshot.freeFireId}
                      </span>
                    )}
                    {previewScreenshot.gameLevel != null && (
                      <span className="font-mono text-blue-300 text-[11px]">Lv.{previewScreenshot.gameLevel}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 bg-black/40 flex flex-col items-center justify-center">
              <div className="relative max-h-[65vh] w-full flex items-center justify-center rounded-xl overflow-hidden border border-white/5 bg-zinc-950">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={resolveAssetUrl(previewScreenshot.url)}
                  alt={`Verification proof for ${previewScreenshot.username}`}
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-white/10 bg-white/[0.02]">
              <a
                href={resolveAssetUrl(previewScreenshot.url)}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1.5 text-xs text-fire-400 hover:text-fire-300 transition-colors"
              >
